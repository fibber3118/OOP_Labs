...
namespace bitarray {
    BitArray::BitArray(int num_bits, unsigned long value) {
        ...
        unsigned long reversed_value = 0;
        for (int i = 0; i < num_bits; i++) {
            if ((value >> i) & 1) {
                reversed_value |= (1UL << (num_bits - 1 - i));
            }
        }
    ...
    std::string BitArray::to_string() const {
        ...
        for (int i = 0; i < num_bits_used / 2; i++) {
            char c = result[i];
            result[i] = result[num_bits_used - 1 - i];
            result[num_bits_used - 1 - i] = c;
        }
        return result;
    }
